...
class StudentParser:
    def __init__(self):
        # 匹配序号的模式：数字+号，或第+数字+号（统一作为序号，不再是学号）
        self.sequence_pattern = re.compile(r'(?:第?)(\d+)(?:号|号学生)')
        # 匹配姓名的模式：2-4个中文字符
    ...
    def parse(self, text: str) -> Optional[Dict[str, any]]:
        ...
        if score_match:
            ...
            # 没有找到"数字+分"，尝试找纯数字（排除序号）
            # 先找出所有数字
            all_numbers = re.findall(r'\d+(?:\.\d+)?', text)
            if not all_numbers:
                return None

            # 如果有序号，排除序号位置的数字，取最后一个数字作为分数
            sequence_match = self.sequence_pattern.search(text)
            if sequence_match and len(all_numbers) > 1:
                # 有序号，取最后一个数字（假设分数在最后）
                try:
    ...
    def parse_multiple(self, text: str) -> List[Dict[str, any]]:
        ...
        if matches:
            # 找到了多个序号+分数对
            for sequence_str, score_str in matches:
                try:
